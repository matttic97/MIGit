...
namespace ReševanjeLabirinta {
    class AStarAlgorithm {
        ...
        public static string Solve(int[,] grid) {
            ...
            start.fScore = GetHeuristicDistance(start, ends);
            ...
            while(openSet.Count > 0) {
                ...
                foreach(Node finish in ends) {
                    if (current.x == finish.x && current.y == finish.y) {
                        return ReturnPath(current, node_count);
                    }
                }

                closedSet.Add(current);
                List<Node> neighbors = GetNeighbors(current, grid, ends);
        ...
        static string ReturnPath(Node node, int node_cont) {
            int steps = 0;
        ...
        static List<Node> GetNeighbors(Node node, int[,] grid, List<Node> ends) {
            int x = node.x, y = node.y;

            List<Node> neighbors = new List<Node>();

            if (x - 1 > 0) {
                Node newNode = CreateNeighbor(x - 1, y, node, ends, grid);
                if(newNode != null) neighbors.Add(newNode);
            }
            if (y - 1 > 0) {
                Node newNode = CreateNeighbor(x, y - 1, node, ends, grid);
                if (newNode != null) neighbors.Add(newNode);
            }
        ...
        static Node CreateNeighbor(int x, int y, Node parent, List<Node> ends, int[,] grid) {
            ...
            newNode.fScore = GetHeuristicDistance(newNode, ends);
            return newNode;
        }
        ...
        static int GetHeuristicDistance(Node node, List<Node> ends) {
            int minDistance = -1;
